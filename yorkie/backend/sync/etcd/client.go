...
const DefaultLockLeaseTimeSec = 30
...
// Lock locks a mutex.
func (lm *internalLocker) Lock(ctx context.Context) error {
	if err := lm.mu.Lock(ctx); err != nil {
		log.Logger.Error(err)
		return err
	}
...
func (lm *internalLocker) Unlock(ctx context.Context) error {
	if err := lm.mu.Unlock(ctx); err != nil {
		log.Logger.Error(err)
		return err
	}
